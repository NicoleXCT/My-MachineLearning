...
testdf = clean_and_munge_data(testdf)
...
x_pre = patsy.build_design_matrices([x_train.design_info], testdf)
x_pre = np.array(x_pre).squeeze()
...
cv=StratifiedShuffleSplit(n_splits=10, test_size=0.2, train_size=None, random_state=seed)).fit(X_train, Y_train)
# 对结果打分
...
y_pre = grid_search.best_estimator_.predict(x_pre)
print(len(np.arange(892,1310)),len(y_pre))
result = pd.DataFrame({'PassengerId':np.arange(892,1310),'Survived':y_pre})
